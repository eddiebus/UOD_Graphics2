...
ComPtr<ID3D11VertexShader> DirectXFramework::CompileVertexShader(wstring FileName, string entryPoint, string shaderTarget)
{
	HRESULT hrResult;

	ComPtr<ID3D10Blob> CompileBlop;
	ComPtr<ID3D10Blob> compileMessageBlob;

	DWORD ShaderCompileFlags = D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION;

	hrResult = D3DCompileFromFile(
		FileName.c_str(),
		nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE,
		entryPoint.c_str(), shaderTarget.c_str(),
		ShaderCompileFlags, NULL,
		CompileBlop.GetAddressOf(),
		compileMessageBlob.GetAddressOf()
	);

	ThrowIfFailed(hrResult);

	ComPtr<ID3D11VertexShader> returnShader;

	hrResult =  _device->CreateVertexShader(
		CompileBlop->GetBufferPointer(),
		CompileBlop->GetBufferSize(), NULL,
		returnShader.GetAddressOf()
	);
...
ComPtr<ID3D11PixelShader> DirectXFramework::CompilePixelShader(wstring FileName, string entryPoint, string shaderTarget)
{
	HRESULT hrResult;
	...
	ComPtr<ID3D11PixelShader> returnShader;

	hrResult = _device->CreatePixelShader(
		CompileBlop->GetBufferPointer(),
		CompileBlop->GetBufferSize(), NULL,
		returnShader.GetAddressOf()
	);
...
double DirectXFramework::GetDeltaTime()
{
	return _deltaTime;
}
...
void DirectXFramework::Update()
{
	...
	std::chrono::steady_clock::duration timeDifference = timeNow - timeLastFrame;
	std::chrono::duration<double> miliSecondDeltaTime = std::chrono::duration_cast<std::chrono::milliseconds>(timeDifference);
	_deltaTime = miliSecondDeltaTime.count();
